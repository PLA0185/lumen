...
**`window_config` 的兼容规则**：窗口配置整体作为一个 JSON 存在
`settings.window_config` 里。结构体带**容器级 `serde(default)`**，
新增字段（例如后加的 `floatingWidth` / `floatingHeight`）在老用户的数据里
不存在时会各自取默认值，而不是让整份配置解析失败、被一次性重置成默认。
写窗口相关的持久化字段时，这条规则必须保持——否则用户调好的置顶、穿透、
快捷键会在升级后集体丢失。尺寸还会在读取时收敛到
260×200 – 1400×1800 的逻辑像素区间内，避免手工改库把窗口弄成 0 尺寸。
