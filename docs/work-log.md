...
- 统一视觉令牌和主界面、任务卡片、对话框、设置页、悬浮窗的交互状态；TaskEditor 折叠低频区域并把标签改为搜索选择。
- 项目、分类、标签分别加载和降级；标签数据失败时基本字段仍可保存，已有标签不会被覆盖。
- 重复范围统计区分计划总数、已生成数和无限系列；修正文案与历史保护说明，时区换成常用名称加自定义 IANA。
- Calendar、Stats、Organize、Dependency search、Floating、Focus 六个消费者统一 latest-wins，并用可控 Promise 顺序证明旧请求不能回写。
- 完成九张真实 WebView2 RC 截图；人工检查发现并修掉任务编辑弹窗的横向滚动条。

### 没做到

- 原生文件选择器仍不能由 CDP 稳定自动选择文件；AI 环境没有真实服务商密钥。这两项不阻断 RC，已写入 `docs/0.4.1-backlog.md`。
- 宽屏内容利用率、月历高密度信息展开、看板低密度布局等 P2/P3 视觉增强没有并入本轮，避免扩大冻结范围。

### 怎么验证的

- 前端：版本一致、类型检查、166 项测试、生产构建、ESLint 全通过。
- Rust：格式检查、全目标检查、361 项库测试、Clippy `-D warnings` 全通过。
- 隔离桌面：architecture 24/24、recurrence 39/39、remediation3 42/42、scale 16/16；成功 profile 均已删除。
- 视觉：浅色 1280×720，深色 1920×1080/2560×1440，以及任务编辑、重复规则、日历、看板、设置和悬浮窗截图逐张检查。

### 相关文档

- `docs/0.4.0-rc-closure.md`
- `docs/0.4.1-backlog.md`
- `docs/screenshots/rc-*.png`

---

## 第 15 轮 · 2026-09-25 · 重复任务同 profile 重启与维护验收（阶段进度）

### 做了

- 新增 `recurrence_restart` 双进程隔离验收：第一个进程从 UI 创建重复任务并修改整系列规则，绑定子任务与附件副本；关闭应用，再用相同临时 profile 启动第二个进程。
- 第二进程核对新规则、原发生与附属记录，并执行一年外范围维护；断言生成新发生、重复维护幂等，孤儿清理后附件副本仍可访问。两个阶段都核对应用实际数据目录，成功后清除任务和 profile。

### 没做到

- 原生文件选择器加入附件仍未自动验收；主验收与重启验收分属两个隔离 profile，还不是任务书要求的单一 16 步连续链路。六视图受控延迟和人工可用性、AI 真实服务商检查仍未完成；版本保持 0.3.0。

### 怎么验证的

- `python tools/run_acceptance.py --exe src-tauri/target/debug/lumen.exe --suite recurrence_restart`：seed 4/4、after 7/7，通过后删除 profile。测试曾因把维护范围选在规则编辑已预建的一年内而失败；改为一年外后断言真实新生成数大于零并通过。主 `recurrence` 验收复跑 39/39；一次并行重负载下范围弹窗尚未就绪导致脚本失败，增加等待与一次重试后单独复跑通过。

### 相关文档

- `docs/ui-ux-audit-0.4.0.md`：补充重启覆盖和原生选择器边界。

---

## 第 14 轮 · 2026-09-25 · 扩展重复任务隔离实机验收（阶段进度）

### 做了

- 将 `verify_recurrence.py` 扩展到 UI 添加子任务、提醒、前置依赖，以及改规则后的身份和附属记录保留；隔离 profile 内的真实后端命令加入 copied 附件，并核对界面显示、规则变更后附件记录、孤儿清理后副本文件。
- 真实点击三种删除范围，进入日历翻看未来三个月，验证停止的系列没有重新生成。成功验收后清理测试任务、前置任务和整个临时 profile。

### 没做到

- Windows 原生文件选择器会弹到当前桌面，验收脚本无法改写 Tauri 的只读调用入口；这轮没有自动选择文件，UI 添加附件不能算通过。重启和后台维护的同一 profile 连续链路也尚未覆盖。
- 六视图受控延迟、AI 真实服务商、大量任务人工可用性等第 13 轮列出的发布前缺口仍在。仍为 0.3.0，未发 0.4.0。

### 怎么验证的

- `python tools/run_acceptance.py --exe src-tauri/target/debug/lumen.exe --suite recurrence`：隔离桌面 38/38，通过后 profile 已删除；失败调试 profile 保留供排查。文件副本保留还由 Rust 集成测试覆盖。

### 相关文档

- `docs/ui-ux-audit-0.4.0.md`：已更新实际覆盖范围与剩余链路。
